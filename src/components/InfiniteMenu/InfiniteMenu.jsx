...
export default function InfiniteMenu({ items = [], scale = 1.0 }) {
  ...
  const sourceItems = useMemo(() => (items.length ? items : defaultItems), [items]);
  ...
  useEffect(() => {
    ...
    const handleActiveItem = index => {
      ...
      setActiveItem(sourceItems[itemIndex]);
    };
  ...
  return (
    <section className="infinite-menu-stage">
      <div className="infinite-menu-stage__canvas-shell">
        ...
        {activeItem ? (
          <>
            ...
            <button type="button" onClick={handleButtonClick} className={`action-button ${isMoving ? 'inactive' : 'active'}`}>
